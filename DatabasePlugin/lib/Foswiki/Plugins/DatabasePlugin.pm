...
package Foswiki::Plugins::DatabasePlugin;
...
sub initPlugin {
    ...
    if ( $Foswiki::cfg{Plugins}{DatabasePlugin}{ConfigSource} eq 'Local' ) {
        ...
        foreach
          my $info ( @{ $Foswiki::cfg{Plugins}{DatabasePlugin}{Databases} } )
        {
            my $dbi = new Foswiki::Plugins::DatabasePlugin::Connection($info);
        ...
        my $sid = "";
        if ( $Foswiki::cfg{Plugins}{DatabasePlugin}{ConfigSID} ) {
            $sid = ";sid=$Foswiki::cfg{Plugins}{DatabasePlugin}{ConfigSID}";
        }
        my $db = DBI->connect(
...
"SELECT description,driver,db_name,db_sid,table_name,ro_username,ro_password,hostname FROM $Foswiki::cfg{Plugins}{DatabasePlugin}{ConfigTable}";
        ...
        while ( my $row = $sth->fetchrow_hashref() ) {
            ...
            my $dbi = new Foswiki::Plugins::DatabasePlugin::Connection($row);
            $dbinfo->{ $dbi->{description} } = $dbi;
        }
    }
...
sub _dispatch {
    ...
    $tag = 'Foswiki::Plugins::DatabasePlugin::' . $tag;

    my $desc = Foswiki::Func::extractNameValuePair( $_[0], 'description' )
      || '';
    my $db = $dbinfo->{$desc};
...
sub commonTagsHandler {
    ...
    $_[0] =~
s/%(DATABASE_REPEAT){(.*?)}%(.*?)%DATABASE_REPEAT%/_dispatch($1, $2, $3)/seog;
    $_[0] =~
s/%(DATABASE_SQL_REPEAT){(.*?)}%(.*?)%DATABASE_SQL_REPEAT%/_dispatch($1, $2, $3)/seog;
