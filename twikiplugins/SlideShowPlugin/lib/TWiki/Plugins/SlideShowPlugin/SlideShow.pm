...
sub handler {
    ...
    if ( $query && $query->param('slideshow') ) {
        ...
        if ( $text =~ /(.*?[\n\r])\-\-\-+(\++)\!* (.*)/s ) {
            ...
            my $hideComments =
              TWiki::Func::getPreferencesValue('SLIDESHOWPLUGIN_HIDECOMMENTS')
              || '';
            ...
            foreach (@slides) {
                ...
                $slideComment = '';
                if (   $hideComments
                    && $slideBody =~ s/(\-\-\-+\+$level+\!*\s*Comments.*)//is )
                {
                    $slideComment = $1;
                }
            ...
            $text =~
s/%TOC(?:\{.*?\})*%/renderSlideToc( $theWeb, $theTopic, @titles )/geo;
            $text .= "\n#GoSlide$slideNum\n%BR%\n";
        }

        $text = "$textPre\n$text\n";
        $text .= renderSlideNav( $theWeb, $theTopic, $slideMax + 1,
            $slideMax, "f p e", $qparams );
        $text .= "\n";
        $text .= "%BR%\n\n" x 30;
        $text =~ s/%BR%/<br \/>/go;
        $text .= $textPost;
...
sub renderSlideNav {
    ...
    if ( $theButtons =~ /f/ ) {
        ...
        $text .=
            "<a href=\"$viewUrl$qstring#GoSlide1\">"
    ...
    if ( $theButtons =~ /p/ ) {
        ...
        $text .=
            "<a href=\"$viewUrl$qstring#GoSlide$prev\">"
    ...
    if ( $theButtons =~ /n/ ) {
        ...
        $text .=
            "<a href=\"$viewUrl$qstring#GoSlide$next\">"
    ...
    if ( $theButtons =~ /l/ ) {
        ...
        $text .=
            "<a href=\"$viewUrl$qstring#GoSlide$theMax\">"
...
sub renderSlideToc {
    ...
    foreach (@theTitles) {
        $text .= "\t\* ";
        $text .=
          "<a href=\"$viewUrl?slideshow=on&amp;skin=print#GoSlide$slideNum\">";
        $text .= " $_ </a>\n";
        $slideNum++;
    }
    return $text;
...
sub readTmplText {
    ...
    unless ($tmplTopic) {
        $theWeb = $installWeb;
        $tmplTopic =
          TWiki::Func::getPreferencesValue("SLIDESHOWPLUGIN_TEMPLATE")
          || "SlideShowPlugin";
    }
    if ( $tmplTopic =~ /^([^\.]+)\.(.*)$/o ) {
        $theWeb    = $1;
    ...
    unless ($text) {
        $text =
            "<font color=\"red\"> $installWeb.SlideShowPlugin Error: </font>"
