					   ...
					   int parent_was_writable)
{
    ...
    dav_error *err;
    if (resource != NULL) {
        /* Change conditions for versioning.
         * In theory, a resource does not become versioned until
         * it is explicitly added to the collection by the client.
         * But we want resources to be auto-versioned. */
         if ((!resource_was_writable ||
              (*vsn_hooks->auto_version_enabled)(resource))
...
struct dav_status_codes {
    int status;
    const char *text;
};
static const struct dav_status_codes dav_code_text[] =
{
    { 100, "Continue" },
...
{
    const struct dav_status_codes *code;

    for (code = dav_code_text; code->status != 0; ++code)
