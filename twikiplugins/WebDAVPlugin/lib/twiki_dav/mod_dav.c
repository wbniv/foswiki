...
static void *dav_create_dir_config(pool *p, char *dir)
{
  ...
  conf->twiki_dir_type = TWIKI_NOTYPE;
...
/* Get the script location for an access */
const char* dav_get_twiki_access_script(const request_rec* r) {
  dav_dir_conf *conf = ap_get_module_config(r->per_dir_config, &dav_module);
  return conf->twiki_access_script;
}

/** See if this is tagged as part of a twiki tree */
int dav_get_twiki_dir_type(const request_rec *r) {
  dav_dir_conf *conf = ap_get_module_config(r->per_dir_config, &dav_module);
  return conf->twiki_dir_type;
}
...
static int dav_method_put(request_rec *r)
{
  ...
  /* This is the only meaningful change from standard dav_fs. Detect a
   * TWiki controlled resource, and tag it as versioned if it is legal
   * to put here */
  if (dav_get_twiki_dir_type(r) && !resource->twiki) {
	/* error */
	return dav_error_response(r, HTTP_CONFLICT,
							  "Cannot PUT at that level in a TWiki.");
  }
...
static int dav_handler(request_rec *r)
{
  ...
  if (r->method_number == M_GET) {
	result = dav_method_get(r);
  } else if (r->method_number == M_PUT) {
	result = dav_method_put(r);
  } else if (r->method_number == M_POST) {
	result = dav_method_post(r);
  } else if (r->method_number == M_DELETE) {
	result = dav_method_delete(r);
  } else if (r->method_number == M_OPTIONS) {
	result = dav_method_options(r);
  } else if (r->method_number == M_PROPFIND) {
	result = dav_method_propfind(r);
  } else if (r->method_number == M_PROPPATCH) {
	result = dav_method_proppatch(r);
  } else if (r->method_number == M_MKCOL) {
	result = dav_method_mkcol(r);
  } else if (r->method_number == M_COPY) {
	result = dav_method_copymove(r, DAV_DO_COPY);
  } else if (r->method_number == M_MOVE) {
	result = dav_method_copymove(r, DAV_DO_MOVE);
  } else if (r->method_number == M_LOCK) {
	result = dav_method_lock(r);
  } else if (r->method_number == M_UNLOCK) {
	result = dav_method_unlock(r);
  } 
  /*
   ...
   */
	...
	result = DECLINED;
  } else if (!strcmp(r->method, "SEARCH")) {
	result = dav_method_search(r);
  } else if (!strcmp(r->method, "CHECKOUT")) {
	result = dav_method_checkout(r);
  } else if (!strcmp(r->method, "UNCHECKOUT")) {
	result = dav_method_uncheckout(r);
  } else if (!strcmp(r->method, "CHECKIN")) {
	result = dav_method_checkin(r);
  } 
#if 0
  else if (!strcmp(r->method, "MKRESOURCE")) {
	result = dav_method_mkresource(r);
  } else if (!strcmp(r->method, "REPORT")) {
	result = dav_method_report(r);
  }
#endif
