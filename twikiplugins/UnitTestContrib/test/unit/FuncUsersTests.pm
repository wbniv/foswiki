...
sub set_up_for_verify {
    ...
    if (   $this->{twiki}->inContext('registration_supported')
        && $this->{twiki}->inContext('registration_enabled') )
    {
        try {
...
sub verify_eachUser {
    ...
    if ( $TWiki::cfg{UserMappingManager} eq 'TWiki::Users::BaseUserMapping' ) {
        @correctList =
          qw/TWikiContributor TWikiGuest TWikiRegistrationAgent UnknownUser/;
    }
    else {
        @correctList =
          qw/TWikiContributor TWikiGuest TWikiRegistrationAgent UnknownUser User86A UserA UserA86 UserB UserC UserZ/;
...
sub verify_eachGroupTraditional {
    ...
    if ( $TWiki::cfg{UserMappingManager} eq 'TWiki::Users::BaseUserMapping' ) {
        ...
        @correctList =
          qw/AdminGroup AandBGroup AandCGroup BandCGroup ScumGroup TWikiAdminGroup TWikiBaseGroup/;
    }
    my $correct = join( ',', sort @correctList );
    $this->assert_str_equals( $correct, $ulist );
}
...
sub verify_eachGroupCustomAdmin {
    ...
    if ( $TWiki::cfg{UserMappingManager} eq 'TWiki::Users::BaseUserMapping' ) {
        ...
        @correctList =
          qw/AdminGroup AandBGroup AandCGroup BandCGroup ScumGroup TWikiBaseGroup/;
    }
...
sub verify_isGroupMember {
    ...
    $this->assert(
        TWiki::Func::isGroupMember(
            'ScumGroup', $TWiki::cfg{DefaultUserWikiName}
        )
    );

    $this->assert( TWiki::Func::isGroupMember( 'ScumGroup', 'UserZ' ) );
    $this->assert(
        TWiki::Func::isGroupMember( 'ScumGroup', $loginname{UserZ} ) );
...
sub verify_eachMembership {
    ...
    $this->assert_str_equals( 'TWikiBaseGroup,ScumGroup',
        sort join( ',', @list ) );
...
sub verify_eachGroupMember {
    ...
    $this->assert_str_equals( "UserA,UserB,$TWiki::cfg{AdminUserWikiName}",
        sort join( ',', @list ) );
    ...
    $this->assert_str_equals( "UserA,$TWiki::cfg{DefaultUserWikiName},UserZ",
        sort join( ',', @list ) );
...
sub verify_getCanonicalUserID_extended {
    ...
    $this->assert_str_equals( $guest_cUID,
        TWiki::Func::getCanonicalUserID( $TWiki::cfg{DefaultUserLogin} ) );
    $this->assert_str_equals( $guest_cUID,
        TWiki::Func::getCanonicalUserID($guest_cUID) );
    $this->assert_str_equals( $guest_cUID,
        TWiki::Func::getCanonicalUserID( $TWiki::cfg{DefaultUserWikiName} ) );
    $this->assert_str_equals(
        $guest_cUID,
        TWiki::Func::getCanonicalUserID(
            $TWiki::cfg{UsersWebName} . '.' . $TWiki::cfg{DefaultUserWikiName}
        )
    );
    ...
    $this->assert_str_equals( $admin_cUID,
        TWiki::Func::getCanonicalUserID($admin_cUID) );
    $this->assert_str_equals( $admin_cUID,
        TWiki::Func::getCanonicalUserID( $TWiki::cfg{AdminUserLogin} ) );
    $this->assert_str_equals( $admin_cUID,
        TWiki::Func::getCanonicalUserID( $TWiki::cfg{AdminUserWikiName} ) );
    $this->assert_str_equals(
        $admin_cUID,
        TWiki::Func::getCanonicalUserID(
            $TWiki::cfg{UsersWebName} . '.' . $TWiki::cfg{AdminUserWikiName}
        )
    );

    my $usera_cUID =
      $this->{twiki}->{users}->getCanonicalUserID( $loginname{UserA} );
    $this->assert_str_equals( $usera_cUID,
        TWiki::Func::getCanonicalUserID($usera_cUID) );
    $this->assert_str_equals( $usera_cUID,
        TWiki::Func::getCanonicalUserID( $loginname{UserA} ) );
    $this->assert_str_equals( $usera_cUID,
        TWiki::Func::getCanonicalUserID('UserA') );
    $this->assert_str_equals(
        $usera_cUID,
        TWiki::Func::getCanonicalUserID(
            $TWiki::cfg{UsersWebName} . '.' . 'UserA'
        )
    );

#            $this->registerUser($loginname{UserA86}, 'User', 'A86', 'user86@example.com');
    my $usera86_cUID =
      $this->{twiki}->{users}->getCanonicalUserID( $loginname{UserA86} );
    $this->assert_str_equals( $usera86_cUID,
        TWiki::Func::getCanonicalUserID($usera86_cUID) );
    $this->assert_str_equals( $usera86_cUID,
        TWiki::Func::getCanonicalUserID( $loginname{UserA86} ) );
    $this->assert_str_equals( $usera86_cUID,
        TWiki::Func::getCanonicalUserID('UserA86') );
    $this->assert_str_equals(
        $usera86_cUID,
        TWiki::Func::getCanonicalUserID(
            $TWiki::cfg{UsersWebName} . '.' . 'UserA86'
        )
    );

#            $this->registerUser($loginname{User86A}, 'User86', 'A', 'user86a@example.com');
    my $user86a_cUID =
      $this->{twiki}->{users}->getCanonicalUserID( $loginname{User86A} );
    $this->assert_str_equals( $user86a_cUID,
        TWiki::Func::getCanonicalUserID($user86a_cUID) );
    $this->assert_str_equals( $user86a_cUID,
        TWiki::Func::getCanonicalUserID( $loginname{User86A} ) );
    $this->assert_str_equals( $user86a_cUID,
        TWiki::Func::getCanonicalUserID('User86A') );
    $this->assert_str_equals(
        $user86a_cUID,
        TWiki::Func::getCanonicalUserID(
            $TWiki::cfg{UsersWebName} . '.' . 'User86A'
        )
    );
...
#            $this->registerUser('86usera', '86User', 'A', 'user86a@example.com');
    ...
    $this->assert_null( $this->{twiki}->{users}
    ...
    $this->assert_null(
        TWiki::Func::getCanonicalUserID(
            $TWiki::cfg{UsersWebName} . '.' . 'NonExistantUser'
        )
    );
    $this->assert_null(
        TWiki::Func::getCanonicalUserID(
            $TWiki::cfg{UsersWebName} . '.' . 'NonExistantUser86'
        )
    );
...
sub verify_getWikiName_extended {
    ...
    $this->assert_str_equals( $TWiki::cfg{DefaultUserWikiName},
    ...
    my $guest_cUID =
      ...
      ->getCanonicalUserID( $TWiki::cfg{DefaultUserLogin} );
    $this->assert_str_equals( $TWiki::cfg{DefaultUserWikiName},
        TWiki::Func::getWikiName( $TWiki::cfg{DefaultUserLogin} ) );
    $this->assert_str_equals(
        $TWiki::cfg{DefaultUserWikiName},
        TWiki::Func::getWikiName($guest_cUID)
    );
    $this->assert_str_equals( $TWiki::cfg{DefaultUserWikiName},
        TWiki::Func::getWikiName( $TWiki::cfg{DefaultUserWikiName} ) );
    $this->assert_str_equals(
        $TWiki::cfg{DefaultUserWikiName},
        TWiki::Func::getWikiName(
            $TWiki::cfg{UsersWebName} . '.' . $TWiki::cfg{DefaultUserWikiName}
    ...
    $this->assert_str_equals( $TWiki::cfg{AdminUserWikiName},
        TWiki::Func::getWikiName($admin_cUID) );
    $this->assert_str_equals( $TWiki::cfg{AdminUserWikiName},
        TWiki::Func::getWikiName( $TWiki::cfg{AdminUserLogin} ) );
    $this->assert_str_equals( $TWiki::cfg{AdminUserWikiName},
        TWiki::Func::getWikiName( $TWiki::cfg{AdminUserWikiName} ) );
    $this->assert_str_equals(
        $TWiki::cfg{AdminUserWikiName},
        TWiki::Func::getWikiName(
            $TWiki::cfg{UsersWebName} . '.' . $TWiki::cfg{AdminUserWikiName}
    ...
    $this->assert_str_equals( 'UserA',
        TWiki::Func::getWikiName( $loginname{UserA} ) );
    $this->assert_str_equals( 'UserA', TWiki::Func::getWikiName('UserA') );
    $this->assert_str_equals( 'UserA',
        TWiki::Func::getWikiName( $TWiki::cfg{UsersWebName} . '.' . 'UserA' ) );

#            $this->registerUser($loginname{UserA86}, 'User', 'A86', 'user86@example.com');
    my $usera86_cUID =
      $this->{twiki}->{users}->getCanonicalUserID( $loginname{UserA86} );
    $this->assert_str_equals( 'UserA86',
        TWiki::Func::getWikiName($usera86_cUID) );
    $this->assert_str_equals( 'UserA86',
        TWiki::Func::getWikiName( $loginname{UserA86} ) );
    $this->assert_str_equals( 'UserA86', TWiki::Func::getWikiName('UserA86') );
    $this->assert_str_equals( 'UserA86',
        TWiki::Func::getWikiName( $TWiki::cfg{UsersWebName} . '.' . 'UserA86' )
    );
...
#            $this->registerUser($loginname{User86A}, 'User86', 'A', 'user86a@example.com');
    ...
    $this->assert_str_equals( 'User86A',
        TWiki::Func::getWikiName($user86a_cUID) );
    $this->assert_str_equals( 'User86A',
        TWiki::Func::getWikiName( $loginname{User86A} ) );
    $this->assert_str_equals( 'User86A', TWiki::Func::getWikiName('User86A') );
    $this->assert_str_equals( 'User86A',
        TWiki::Func::getWikiName( $TWiki::cfg{UsersWebName} . '.' . 'User86A' )
    );
...
#            $this->registerUser('86usera', '86User', 'A', 'user86a@example.com');
    ...
    $this->assert_str_equals( $TWiki::cfg{DefaultUserWikiName},
        TWiki::Func::getWikiName($nonexistantuser_cUID) );
    $this->assert_str_equals( $loginname{NonExistantuser},
        TWiki::Func::getWikiName( $loginname{NonExistantuser} ) );
    $this->assert_str_equals( 'NonExistantUser',
        TWiki::Func::getWikiName('NonExistantUser') );
    $this->assert_str_equals(
        'NonExistantUser',
        TWiki::Func::getWikiName(
            $TWiki::cfg{UsersWebName} . '.' . 'NonExistantUser'
...
sub verify_getWikiUserName_extended {
    ...
    $this->assert_str_equals(
        $TWiki::cfg{UsersWebName} . '.' . $TWiki::cfg{DefaultUserWikiName},
        TWiki::Func::getWikiUserName( $TWiki::cfg{DefaultUserLogin} )
    );
    $this->assert_str_equals(
        $TWiki::cfg{UsersWebName} . '.' . $TWiki::cfg{DefaultUserWikiName},
        TWiki::Func::getWikiUserName($guest_cUID) );
    $this->assert_str_equals(
        $TWiki::cfg{UsersWebName} . '.' . $TWiki::cfg{DefaultUserWikiName},
        TWiki::Func::getWikiUserName( $TWiki::cfg{DefaultUserWikiName} )
    );
    $this->assert_str_equals(
        $TWiki::cfg{UsersWebName} . '.' . $TWiki::cfg{DefaultUserWikiName},
        TWiki::Func::getWikiUserName(
    ...
    $this->assert_str_equals(
        $TWiki::cfg{UsersWebName} . '.' . $TWiki::cfg{AdminUserWikiName},
        TWiki::Func::getWikiUserName($admin_cUID) );
    $this->assert_str_equals(
        $TWiki::cfg{UsersWebName} . '.' . $TWiki::cfg{AdminUserWikiName},
        TWiki::Func::getWikiUserName( $TWiki::cfg{AdminUserLogin} )
    );
    $this->assert_str_equals(
        $TWiki::cfg{UsersWebName} . '.' . $TWiki::cfg{AdminUserWikiName},
        TWiki::Func::getWikiUserName( $TWiki::cfg{AdminUserWikiName} )
    );
    $this->assert_str_equals(
        $TWiki::cfg{UsersWebName} . '.' . $TWiki::cfg{AdminUserWikiName},
        TWiki::Func::getWikiUserName(
    ...
    $this->assert_str_equals(
        $TWiki::cfg{UsersWebName} . '.' . 'UserA',
        TWiki::Func::getWikiUserName($usera_cUID)
    );
    $this->assert_str_equals(
        $TWiki::cfg{UsersWebName} . '.' . 'UserA',
        TWiki::Func::getWikiUserName( $loginname{UserA} )
    );
    $this->assert_str_equals(
        $TWiki::cfg{UsersWebName} . '.' . 'UserA',
        TWiki::Func::getWikiUserName('UserA')
    );
    $this->assert_str_equals(
        $TWiki::cfg{UsersWebName} . '.' . 'UserA',
        TWiki::Func::getWikiUserName(
...
#            $this->registerUser($loginname{UserA86}, 'User', 'A86', 'user86@example.com');
    ...
    $this->assert_str_equals(
        $TWiki::cfg{UsersWebName} . '.' . 'UserA86',
        TWiki::Func::getWikiUserName($usera86_cUID)
    );
    $this->assert_str_equals(
        $TWiki::cfg{UsersWebName} . '.' . 'UserA86',
        TWiki::Func::getWikiUserName( $loginname{UserA86} )
    );
    $this->assert_str_equals(
        $TWiki::cfg{UsersWebName} . '.' . 'UserA86',
        TWiki::Func::getWikiUserName('UserA86')
    );
    $this->assert_str_equals(
        $TWiki::cfg{UsersWebName} . '.' . 'UserA86',
        TWiki::Func::getWikiUserName(
...
#            $this->registerUser($loginname{User86A}, 'User86', 'A', 'user86a@example.com');
    ...
    $this->assert_str_equals(
        $TWiki::cfg{UsersWebName} . '.' . 'User86A',
        TWiki::Func::getWikiUserName($user86a_cUID)
    );
    $this->assert_str_equals(
        $TWiki::cfg{UsersWebName} . '.' . 'User86A',
        TWiki::Func::getWikiUserName( $loginname{User86A} )
    );
    $this->assert_str_equals(
        $TWiki::cfg{UsersWebName} . '.' . 'User86A',
        TWiki::Func::getWikiUserName('User86A')
    );
    $this->assert_str_equals(
        $TWiki::cfg{UsersWebName} . '.' . 'User86A',
        TWiki::Func::getWikiUserName(
...
#            $this->registerUser('86usera', '86User', 'A', 'user86a@example.com');
    ...
    #TODO: consider how to render unkown user's
    $this->assert_str_equals(
        $TWiki::cfg{UsersWebName} . '.' . 'NonExistantUserAsdf',
        TWiki::Func::getWikiUserName('NonExistantUserAsdf')
    );
    my $nonexistantuser_cUID =
      $this->{twiki}->{users}->getCanonicalUserID('nonexistantuserasdf');
    $this->annotate($nonexistantuser_cUID);    #returns guest
    $this->assert_str_equals(
        $TWiki::cfg{UsersWebName} . '.' . $TWiki::cfg{DefaultUserWikiName},
        TWiki::Func::getWikiUserName($nonexistantuser_cUID) );
    $this->assert_str_equals(
        $TWiki::cfg{UsersWebName} . '.' . 'nonexistantuserasdf',
        TWiki::Func::getWikiUserName('nonexistantuserasdf')
    );
    $this->assert_str_equals(
        $TWiki::cfg{UsersWebName} . '.' . 'nonexistantuserasdfqwer',
        TWiki::Func::getWikiUserName('nonexistantuserasdfqwer')
    );
    $this->assert_str_equals(
        $TWiki::cfg{UsersWebName} . '.' . 'NonExistantUserAsdf',
        TWiki::Func::getWikiUserName('NonExistantUserAsdf')
    ...
    $this->assert_str_equals(
        $TWiki::cfg{UsersWebName} . '.' . 'NonExistantUserAsdf86',
        TWiki::Func::getWikiUserName(
            $TWiki::cfg{UsersWebName} . '.' . 'NonExistantUserAsdf86'
        )
    );
...
sub verify_wikiToUserName_extended {
    ...
    my $guest_cUID =
      $this->{twiki}->{users}
      ->getCanonicalUserID( $TWiki::cfg{DefaultUserLogin} );
    $this->assert_str_equals( $TWiki::cfg{DefaultUserLogin},
        TWiki::Func::wikiToUserName($guest_cUID) );
    $this->assert_str_equals( $TWiki::cfg{DefaultUserLogin},
        TWiki::Func::wikiToUserName( $TWiki::cfg{DefaultUserLogin} ) );
    $this->assert_str_equals( $TWiki::cfg{DefaultUserLogin},
        TWiki::Func::wikiToUserName( $TWiki::cfg{DefaultUserWikiName} ) );
    $this->assert_str_equals(
    ...
    my $admin_cUID =
      $this->{twiki}->{users}
      ->getCanonicalUserID( $TWiki::cfg{AdminUserLogin} );
    $this->assert_str_equals( $TWiki::cfg{AdminUserLogin},
        TWiki::Func::wikiToUserName($admin_cUID) );
    $this->assert_str_equals( $TWiki::cfg{AdminUserLogin},
        TWiki::Func::wikiToUserName( $TWiki::cfg{AdminUserLogin} ) );
    $this->assert_str_equals( $TWiki::cfg{AdminUserLogin},
        TWiki::Func::wikiToUserName( $TWiki::cfg{AdminUserWikiName} ) );
    $this->assert_str_equals(
        $TWiki::cfg{AdminUserLogin},
        TWiki::Func::wikiToUserName(
            $TWiki::cfg{UsersWebName} . '.' . $TWiki::cfg{AdminUserWikiName}
    ...
    $this->assert_str_equals( $loginname{UserA},
        TWiki::Func::wikiToUserName($usera_cUID) );
    $this->assert_str_equals( $loginname{UserA},
        TWiki::Func::wikiToUserName( $loginname{UserA} ) );
    $this->assert_str_equals( $loginname{UserA},
        TWiki::Func::wikiToUserName('UserA') );
    $this->assert_str_equals(
        $loginname{UserA},
        TWiki::Func::wikiToUserName(
            $TWiki::cfg{UsersWebName} . '.' . 'UserA'
...
#            $this->registerUser($loginname{UserA86}, 'User', 'A86', 'user86@example.com');
    ...
    $this->assert_str_equals( $loginname{UserA86},
        TWiki::Func::wikiToUserName($usera86_cUID) );
    $this->assert_str_equals( $loginname{UserA86},
        TWiki::Func::wikiToUserName( $loginname{UserA86} ) );
    $this->assert_str_equals( $loginname{UserA86},
        TWiki::Func::wikiToUserName('UserA86') );
    $this->assert_str_equals(
        $loginname{UserA86},
        TWiki::Func::wikiToUserName(
            $TWiki::cfg{UsersWebName} . '.' . 'UserA86'
...
#            $this->registerUser($loginname{User86A}, 'User86', 'A', 'user86a@example.com');
    ...
    $this->assert_str_equals( $loginname{User86A},
        TWiki::Func::wikiToUserName($user86a_cUID) );
    $this->assert_str_equals( $loginname{User86A},
        TWiki::Func::wikiToUserName( $loginname{User86A} ) );
    $this->assert_str_equals( $loginname{User86A},
        TWiki::Func::wikiToUserName('User86A') );
    $this->assert_str_equals(
        $loginname{User86A},
        TWiki::Func::wikiToUserName(
            $TWiki::cfg{UsersWebName} . '.' . 'User86A'
...
sub verify_isAnAdmin_extended {
    ...
    $this->assert(
        !TWiki::Func::isAnAdmin( $TWiki::cfg{DefaultUserWikiName} ) );
    $this->assert(
    ...
    $this->assert(
        TWiki::Func::isAnAdmin( $TWiki::cfg{UsersWebName} . '.' . 'UserA' ) );

    $this->assert(
        !TWiki::Func::isAnAdmin( $TWiki::cfg{UsersWebName} . '.' . 'UserB' ) );
    my $userb_cUID =
      $this->{twiki}->{users}->getCanonicalUserID( $loginname{UserB} );
...
sub verify_isGroupMember_extended {
    ...
    $this->assert(
        !TWiki::Func::isGroupMember(
            'AandBGroup', $TWiki::cfg{DefaultUserLogin}
        )
    );
    $this->assert( !TWiki::Func::isGroupMember( 'AandBGroup', $guest_cUID ) );
    $this->assert(
        !TWiki::Func::isGroupMember(
            'AandBGroup', $TWiki::cfg{DefaultUserWikiName}
        )
    );
    $this->assert(
        !TWiki::Func::isGroupMember(
    ...
    $this->assert(
        TWiki::Func::isGroupMember( 'AandBGroup', $TWiki::cfg{AdminUserLogin} )
    );
    $this->assert(
        TWiki::Func::isGroupMember(
            'AandBGroup', $TWiki::cfg{AdminUserWikiName}
        )
    );
    $this->assert(
        TWiki::Func::isGroupMember(
    ...
    $this->assert(
        !TWiki::Func::isGroupMember(
            'AandCGroup', $TWiki::cfg{AdminUserLogin}
        )
    );
    $this->assert(
        !TWiki::Func::isGroupMember(
            'AandCGroup', $TWiki::cfg{AdminUserWikiName}
        )
    );
    $this->assert(
        !TWiki::Func::isGroupMember(
    ...
    $this->assert(
        TWiki::Func::isGroupMember( 'AandBGroup', $loginname{UserA} ) );
    $this->assert( TWiki::Func::isGroupMember( 'AandBGroup', 'UserA' ) );
    $this->assert(
        TWiki::Func::isGroupMember(
            'AandBGroup', $TWiki::cfg{UsersWebName} . '.' . 'UserA'
        )
    );
    ...
    $this->annotate($nonexistantuser_cUID);
    $this->assert(
        !TWiki::Func::isGroupMember( 'AandBGroup', $nonexistantuser_cUID ) );
    $this->assert(
        !TWiki::Func::isGroupMember(
    ...
    $this->assert(
        !TWiki::Func::isGroupMember( 'AandBGroup', 'NonExistantUser' ) );
    $this->assert(
        !TWiki::Func::isGroupMember(
            'AandBGroup', $TWiki::cfg{UsersWebName} . '.' . 'NonExistantUser'
        )
    );
...
#$this->assert(!TWiki::Func::isGroupMember('AandBGroup', $TWiki::cfg{UsersWebName}.'.'.'AandBGroup'));
    ...
    $this->assert(
        !TWiki::Func::isGroupMember(
            $TWiki::cfg{SuperAdminGroup}, $guest_cUID
        )
    );
    $this->assert(
        !TWiki::Func::isGroupMember(
    ...
    $this->assert(
        TWiki::Func::isGroupMember( $TWiki::cfg{SuperAdminGroup}, $admin_cUID )
    );
    $this->assert(
        TWiki::Func::isGroupMember(
            $TWiki::cfg{SuperAdminGroup},
    ...
    $this->assert(
        TWiki::Func::isGroupMember( $TWiki::cfg{SuperAdminGroup}, $usera_cUID )
    );
    $this->assert(
        TWiki::Func::isGroupMember(
            $TWiki::cfg{SuperAdminGroup},
    ...
    $this->assert(
        TWiki::Func::isGroupMember( $TWiki::cfg{SuperAdminGroup}, 'UserA' ) );
    $this->assert(
        TWiki::Func::isGroupMember(
            $TWiki::cfg{SuperAdminGroup},
            $TWiki::cfg{UsersWebName} . '.' . 'UserA'
    ...
    $this->assert(
        !TWiki::Func::isGroupMember(
            $TWiki::cfg{SuperAdminGroup}, $userb_cUID
        )
    );
    $this->assert(
        !TWiki::Func::isGroupMember( $TWiki::cfg{SuperAdminGroup}, 'UserB' ) );
    $this->assert(
        !TWiki::Func::isGroupMember(
            $TWiki::cfg{SuperAdminGroup},
            $TWiki::cfg{UsersWebName} . '.' . 'UserB'
        )
    );
