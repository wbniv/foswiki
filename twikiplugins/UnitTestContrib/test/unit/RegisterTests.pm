...
EOF
        ...
        $this->{twiki}->{store}->saveTopic( $this->{twiki}->{user},
            $this->{users_web}, $TWiki::cfg{SuperAdminGroup}, <<EOF);
        ...
        foreach my $mail (@TWikiFnTestCase::mails) {
            if ( $mail =~ /^Subject:.*Registration for/m ) {
                if ( $mail =~ /^To: .*\b$this->{new_user_email}\b/m ) {
                    ...
                    $this->assert_matches(
qr/To: $TWiki::cfg{WebMasterName} <$TWiki::cfg{WebMasterEmail}>/,
                        $mail
                    );
                }
            }
    ...
    $this->assert(
        $this->{twiki}->{store}->topicExists(
            $TWiki::cfg{UsersWebName}, $this->{new_user_wikiname}
        )
    );
}
###################################
...
sub verify_userTopicWithPMWithoutForm {
    ...
    $this->assert( $text =~ s/^\s*\* Name: $this->{new_user_fullname}$//m,
        $text );
    $this->assert(
        $text =~ s/$TWiki::cfg{UsersWebName}\.$this->{new_user_wikiname}//,
        $text );
...
sub verify_userTopicWithoutPMWithForm {
    ...
    my $m =
      new TWiki::Meta( $this->{twiki}, $this->{users_web}, 'NewUserTemplate' );
...
sub verify_registerBadVerify {
    ...
    $this->assert_matches(
        qr/From: $TWiki::cfg{WebMasterName} <$TWiki::cfg{WebMasterEmail}>/,
        $mess );
    $this->assert_matches( qr/To: .*\b$this->{new_user_email}\b/, $mess );

    # check the verification code
    $this->assert_matches( qr/'$this->{twiki}->{DebugVerificationCode}'/,
        $mess );
}
...
sub verify_UnregisteredUser {
    ...
    my $result2 =
      TWiki::UI::Register::_loadPendingRegistration( $session, "GitWit.0" );
...
sub verify_missingElements {
    ...
    $this->assert_deep_equals(
        [ TWiki::UI::Register::_missingElements( \@present, \@required ) ],
        ["six"] );
    $this->assert_deep_equals(
        [ TWiki::UI::Register::_missingElements( \@present, \@present ) ], [] );
}
...
EOM
    ...
    my $file =
      $TWiki::cfg{DataDir} . '/' . $this->{test_web} . '/' . $regTopic . '.txt';
...
EOM
    ...
    my $actual =
      TWiki::UI::Register::_buildConfirmationEmail( $this->{twiki}, \%data,
...
sub verify_resetPassword_NoTWikiUsersEntry {
    ...
    $this->{twiki}->{store}->moveTopic(
        ...
        $TWiki::cfg{UsersWebName},
        $TWiki::cfg{UsersTopicName} . 'DELETED',
