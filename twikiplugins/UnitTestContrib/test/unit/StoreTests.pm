...
sub test_CreateEmptyWeb {
    ...
    $this->assert(
      ...
      ;    #TODO: how can this succeed without a user? to check perms?
...
#TODO how should this fail if we are testing a store impl that does not have a _deault web ?
    $this->{twiki}->{store}
      ->createWeb( $this->{twiki}->{user}, $web, '_default' );
    $this->assert( $this->{twiki}->{store}->webExists($web) );
    my @topics        = $this->{twiki}->{store}->getTopicNames($web);
...
sub test_CreateWebWithNonExistantBaseWeb {
    ...
    try {
        $this->{twiki}->{store}
          ->createWeb( $this->{twiki}->{user}, $web, 'DoesNotExists' );
    }
...
sub test_CreateSimpleTopic {
    ...
    $this->{twiki}->{store}
      ->saveTopic( $this->{test_user_login}, $web, $topic, $text, $meta );
    $this->assert( $this->{twiki}->{store}->topicExists( $web, $topic ) );
    my ( $readMeta, $readText ) =
      $this->{twiki}->{store}
      ->readTopic( $this->{test_user_login}, $web, $topic );
...
sub test_getRevisionInfo {
    ...
    $this->assert_equals( 1,
        $this->{twiki}->{store}->getRevisionNumber( $web, $topic ) );

    $text .= "\nnewline";
    $this->{twiki}->{store}->saveTopic( $this->{test_user_login},
        $web, $topic, $text, $meta, { forcenewrevision => 1 } );
    ...
    $this->assert_equals( 2,
        $this->{twiki}->{store}->getRevisionNumber( $web, $topic ) );
    my ( $infodate, $infouser, $inforev, $infocomment ) =
      $this->{twiki}->{store}->getRevisionInfo( $web, $topic );
...
"This is some test text\n   * some list\n   * $topic\n   * content\n :) :)";
    $this->{twiki}->{store}
      ->saveTopic( $this->{test_user_login}, $web, $topic . 'a', $text, $meta );
    $text =
"This is some test text\n   * some list\n   * $topic\n   * content\n :) :)";
    $this->{twiki}->{store}
      ->saveTopic( $this->{test_user_login}, $web, $topic . 'b', $text, $meta );
    $text =
"This is some test text\n   * some list\n   * $topic\n   * content\n :) :)";
    $this->{twiki}->{store}
      ->saveTopic( $this->{test_user_login}, $web, $topic . 'c', $text, $meta );

    $this->{twiki}->{store}->moveTopic( $web, $topic, $web, 'TopicMovedToHere',
...
sub test_beforeSaveHandlerChangeText {
    ...
    push(
        @{
            $this->{twiki}->{plugins}->{registeredHandlers}{beforeSaveHandler}
          },
...
sub test_attachmentSaveHandlers {
    ...
    $this->{twiki}->{store}
      ->createWeb( $this->{twiki}->{user}, $web, '_default' );
    $this->{twiki}->{store}
      ->saveTopic( $this->{twiki}->{user}, $web, $topic, "", undef );
    ...
    my $text =
      $this->{twiki}->{store}
      ->readAttachment( $this->{twiki}->{user}, $web, $topic, "testfile.gif" );
...
sub test_eachChange {
    ...
    my $start = time();
    $this->{twiki}->{store}
      ->saveTopic( $this->{twiki}->{user}, $web, "ClutterBuck", "One" );
    $this->{twiki}->{store}
      ->saveTopic( $this->{twiki}->{user}, $web, "PiggleNut", "One" );

    # Wait a second
    sleep(1);
    my $mid = time();
    $this->{twiki}->{store}->saveTopic( $this->{twiki}->{user},
        $web, "ClutterBuck", "One", undef, { forcenewrevision => 1 } );
    $this->{twiki}->{store}->saveTopic( $this->{twiki}->{user},
        $web, "PiggleNut", "Two", undef, { forcenewrevision => 1 } );
    my $change;
