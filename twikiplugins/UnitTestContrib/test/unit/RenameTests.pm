...
sub set_up {
    ...
    $this->{twiki} = new TWiki( $this->{test_user_login},
        new CGI( { topic => "/$this->{test_web}/OldTopic" } ) );

    $this->{new_web} = $this->{test_web} . 'New';
    $this->{twiki}->{store}
      ->createWeb( $this->{twiki}->{user}, $this->{new_web} );
...
THIS
    ...
    foreach my $topic ( 'OldTopic', 'OtherTopic', 'random', 'Random', 'ranDom' )
    {
        ...
        $this->{twiki}->{store}->saveTopic( $this->{twiki}->{user},
            $this->{test_web}, $topic, $originaltext, $meta );
    }

    my $meta =
      new TWiki::Meta( $this->{twiki}, $this->{new_web}, 'OtherTopic' );
    $meta->putKeyed(
        'FIELD',
        {
            name  => $this->{test_web},
    ...
    $meta->put( "TOPICPARENT", { name => "$this->{test_web}.OldTopic" } );
    $this->{twiki}->{store}->saveTopic( $this->{twiki}->{user},
        $this->{new_web}, 'OtherTopic', $originaltext, $meta );
...
sub checkReferringTopics {
    ...
    my $refs = TWiki::UI::Manage::getReferringTopics( $this->{twiki},
        $web, $topic, $all );
...
sub test_referringTopicsThisWeb {
    ...
    my $lott = lc($ott);
    $this->{twiki}->{store}->saveTopic( $this->{twiki}->{user},
        $this->{test_web}, 'MatchMeOne', <<THIS );
[[$ott]]
THIS
    $this->{twiki}->{store}->saveTopic( $this->{twiki}->{user},
        $this->{test_web}, 'MatchMeTwo', <<THIS );
[[$lott]]
THIS
    $this->{twiki}->{store}->saveTopic( $this->{twiki}->{user},
        $this->{new_web}, 'MatchMeThree', <<THIS );
[[$this->{test_web}.$ott]]
THIS
    $this->{twiki}->{store}->saveTopic( $this->{twiki}->{user},
        $this->{new_web}, 'MatchMeFour', <<THIS );
[[$this->{test_web}.$lott]]
THIS
    $this->{twiki}->{store}->saveTopic( $this->{twiki}->{user},
        $this->{test_web}, 'NoMatch', <<THIS );
Refer to $ott and $lott
THIS
    $this->{twiki}->{store}
      ->saveTopic( $this->{twiki}->{user}, $this->{new_web}, 'NoMatch',
        <<THIS );
Refer to $ott and $lott
THIS
...
THIS
    $this->{twiki}->{store}->saveTopic( $this->{twiki}->{user},
        $this->{test_web}, 'MatchMeThree', <<THIS );
Random Random Random
THIS
    $this->{twiki}->{store}->saveTopic( $this->{twiki}->{user},
        $this->{test_web}, 'MatchMeFour', <<THIS );
RanDom RanDom RanDom
THIS
    $this->{twiki}->{store}->saveTopic( $this->{twiki}->{user},
        $this->{test_web}, 'MatchMeFive', <<THIS );
[[random]]
THIS
    $this->{twiki}->{store}->saveTopic( $this->{twiki}->{user},
        $this->{test_web}, 'MatchMeSix', <<THIS );
[[ranDom]]
THIS
    $this->{twiki}->{store}->saveTopic( $this->{twiki}->{user},
        $this->{test_web}, 'MatchMeSeven', <<THIS );
[[Random]]
THIS
    $this->{twiki}->{store}->saveTopic( $this->{twiki}->{user},
        $this->{test_web}, 'MatchMeEight', <<THIS );
[[RanDom]]
THIS
...
sub test_rename_oldwebnewtopic {
    ...
    $this->assert(
        $this->{twiki}->{store}->topicExists( $this->{test_web}, 'NewTopic' ) );
    $this->assert(
        !$this->{twiki}->{store}->topicExists( $this->{test_web}, 'OldTopic' )
    );
...
sub test_rename_newweboldtopic {
    ...
    $this->assert(
        $this->{twiki}->{store}->topicExists( $this->{new_web}, 'OldTopic' ) );
    $this->assert(
        !$this->{twiki}->{store}->topicExists( $this->{test_web}, 'OldTopic' )
    );
...
sub test_rename_from_lowercase {
    my $this = shift;
    my $meta =
      new TWiki::Meta( $this->{twiki}, $this->{test_web}, 'lowercase' );
    my $topictext = <<THIS;
One lowercase
Twolowercase
[[lowercase]]
THIS
    $this->{twiki}->{store}->saveTopic( $this->{twiki}->{user},
        $this->{test_web}, 'lowercase', $topictext, $meta );
    my $query = new CGI(
        {
            action           => 'rename',
            topic            => 'lowercase',
...
sub test_accessRenameRestrictedTopic {
    my $this = shift;
    my $meta = new TWiki::Meta( $this->{twiki}, $this->{test_web}, 'OldTopic' );
    my $topictext = "   * Set ALLOWTOPICRENAME = GungaDin\n";
    $this->{twiki}->{store}->saveTopic( $this->{twiki}->{user},
        $this->{test_web}, 'OldTopic', $topictext, $meta );
    my $query = new CGI(
        {
            action   => 'rename',
            topic    => 'OldTopic',
...
sub test_accessRenameRestrictedWeb {
    ...
    $this->{twiki}->{store}->saveTopic(
        $this->{twiki}->{user},
        $this->{test_web}, $TWiki::cfg{WebPrefsTopicName},
        $topictext, $meta
    );
    my $query = new CGI(
        {
...
sub test_leaseReleasemeLetMeGo {
    ...
    # Grab a lease
    $this->{twiki}->{store}
      ->setLease( $this->{test_web}, 'OldTopic', $this->{twiki}->{user}, 1000 );
    ...
    my $lease =
      $this->{twiki}->{store}->getLease( $this->{test_web}, 'OldTopic' );
