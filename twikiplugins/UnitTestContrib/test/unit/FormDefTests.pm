...
FORM
    $this->{twiki}->{store}->saveTopic( $this->{twiki}->{user},
        $this->{test_web}, 'ValsElsewhere', <<FORM);
| *Name* |
| ValOne |
| RowName |
| Age |
...
FORM
    $this->{twiki}->{store}
      ->saveTopic( $this->{twiki}->{user}, $this->{test_web}, 'Splodge',
        <<FORM);
| *Name* |
| ValOne |
| RowName |
...
FORM
    $this->{twiki}->{store}->saveTopic( $this->{twiki}->{user},
        $this->{test_web}, 'SplodgeOne', <<FORM);
| Age |
FORM
    $this->{twiki}->{store}->saveTopic( $this->{twiki}->{user},
        $this->{test_web}, 'SplodgeTwo', <<FORM);
| Beauty |
FORM
    my $def = new TWiki::Form( $this->{twiki}, $this->{test_web}, 'TestForm' );
...
FORM
    ...
    my ( $meta, $text ) =
      $this->{twiki}->{store}
      ->readTopic( undef, $this->{test_web}, 'TestForm' );
