...
sub set_up {
    ...
    $this->assert_null( $this->{twiki}->{store}
          ->createWeb( $this->{twiki}->{user}, $this->{test_web2} ) );
}
...
sub test_w2em {
    ...
    my $ems =
      join( ',', $this->{twiki}->{users}->getEmails( $this->{twiki}->{user} ) );
...
sub test_eachChangeSince {
    ...
    $this->{twiki}->{store}
      ->saveTopic( $gus, $this->{test_web}, "ClutterBuck", "One" );
    $this->{twiki}->{store}
      ->saveTopic( $sb, $this->{test_web}, "PiggleNut", "One" );

    # Wait a second
    sleep(1);
    my $mid = time();
    $this->{twiki}->{store}
      ->saveTopic( $sb, $this->{test_web}, "ClutterBuck", "One" );
    $this->{twiki}->{store}
      ->saveTopic( $gus, $this->{test_web}, "PiggleNut", "Two", undef );
    my $change;
