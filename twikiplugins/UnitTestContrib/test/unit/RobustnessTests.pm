...
sub test_normalize {
    ...
    $this->assert_str_equals( "abc",
        TWiki::Sandbox::normalizeFileName("./abc") );
    $this->assert_str_equals( "abc",
        TWiki::Sandbox::normalizeFileName("abc/.") );
    ...
    $this->assert_str_equals( "${slash}a${slash}bc",
        TWiki::Sandbox::normalizeFileName("/a/bc") );
    $this->assert_str_equals( "${slash}a${slash}b${slash}c",
        TWiki::Sandbox::normalizeFileName("/a/b/c") );
    $this->assert_str_equals( "${slash}a${slash}b${slash}c",
        TWiki::Sandbox::normalizeFileName("/a/b/c/") );

    $this->assert_str_equals( "abc",
        TWiki::Sandbox::normalizeFileName(".${slash}abc") );
    $this->assert_str_equals( "abc",
        TWiki::Sandbox::normalizeFileName("abc${slash}.") );
    $this->assert_str_equals( "${slash}abc",
        TWiki::Sandbox::normalizeFileName("${slash}${slash}abc") );
    $this->assert_str_equals( "${slash}a${slash}bc",
        TWiki::Sandbox::normalizeFileName("${slash}a${slash}bc") );
    $this->assert_str_equals( "${slash}a${slash}b${slash}c",
        TWiki::Sandbox::normalizeFileName("${slash}a${slash}b${slash}c") );
    $this->assert_str_equals(
        "${slash}a${slash}b${slash}c",
        TWiki::Sandbox::normalizeFileName(
    ...
    unless ( $TWiki::cfg{OS} eq 'WINDOWS' ) {
        ...
        $this->assert_str_equals( "${slash}a${slash}bc",
            TWiki::Sandbox::normalizeFileName("${slash}${slash}a${slash}bc") );
        $this->assert_str_equals( "${slash}abc",
            TWiki::Sandbox::normalizeFileName("${slash}abc") );
        $this->assert_str_equals( "${slash}a${slash}b${slash}c",
            TWiki::Sandbox::normalizeFileName("//a/b/c/") );
    ...
    $this->assert_str_equals( "a${slash}..b",
        TWiki::Sandbox::normalizeFileName("a/..b") );
}
...
sub test_sanitizeAttachmentName {
    ...
    $this->assert_str_equals( "abc",
        TWiki::Sandbox::sanitizeAttachmentName("abc") );
    $this->assert_str_equals( "abc.txt",
        TWiki::Sandbox::sanitizeAttachmentName("abc.txt") );
    $this->assert_str_equals( "abc.txt",
        TWiki::Sandbox::sanitizeAttachmentName("../abc.txt") );
    $this->assert_str_equals( "abc.txt",
        TWiki::Sandbox::sanitizeAttachmentName(".abc.txt") );
    ...
    $this->assert_str_equals( $untainted,
        TWiki::Sandbox::sanitizeAttachmentName($tainted) );
}
...
sub test_buildCommandLine {
    ...
    $this->assert_deep_equals(
        [ 1, 2, 3 ],
        [
            $TWiki::sandbox->_buildCommandLine(
                " %A%  %B%  %C% ",
                ( A => 1, B => 2, C => 3 )
            )
        ]
    ...
    $this->assert_deep_equals(
        ...
        [
            $TWiki::sandbox->_buildCommandLine(
                "%A|D%",
                A => TWiki::Time::formatTime( 1100944661, '$rcs', 'gmtime' )
            )
        ]
    );
...
sub verify {
    my $this = shift;
    my ( $out, $exit ) =
      $TWiki::sandbox->sysCommand( 'sh -c %A%', A => 'echo OK; echo BOSS' );
    $this->assert_str_equals( "OK\nBOSS\n", $out );
    $this->assert_equals( 0, $exit );
    ( $out, $exit ) = $TWiki::sandbox->sysCommand( 'sh -c %A%',
    ...
    $this->assert_str_equals( "", $out );
    ( $out, $exit ) =
      $TWiki::sandbox->sysCommand( 'sh -c %A%', A => 'echo urmf; exit 7' );
    $this->assert( $exit != 0 );
