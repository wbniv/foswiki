...
sub new {
    my $self = shift()->SUPER::new( 'SaveScript', @_ );
    return $self;
}
...
sub set_up {
    ...
    $this->{twiki}->{store}->saveTopic( $this->{test_user_login},
        $this->{test_web}, 'TestForm1', $testform1, undef );

    $this->{twiki}->{store}->saveTopic( $this->{test_user_2_login},
        $this->{test_web}, 'TestForm2', $testform2, undef );

    $this->{twiki}->{store}->saveTopic( $this->{test_user_login},
        $this->{test_web}, 'TestForm3', $testform3, undef );

    $this->{twiki}->{store}->saveTopic( $this->{test_user_login},
        $this->{test_web}, 'TestForm4', $testform4, undef );
...
sub test_AUTOINC {
    ...
    my %old;
    foreach
      my $t ( $this->{twiki}->{store}->getTopicNames( $this->{test_web} ) )
    {
        $old{$t} = 1;
    }
...
sub test_emptySave {
    ...
    my ( $meta, $text ) =
      $this->{twiki}->{store}
      ->readTopic( undef, $this->{test_web}, 'EmptyTestSaveScriptTopic' );
...
sub test_templateTopicTextSave {
    ...
    my ( $meta, $text ) =
      $this->{twiki}->{store}
      ->readTopic( undef, $this->{test_web}, 'TemplateTopic' );
...
sub test_prevTopicTextSave {
    ...
    my ( $meta, $text ) =
      $this->{twiki}->{store}
      ->readTopic( undef, $this->{test_web}, 'PrevTopicTextSave' );
    $this->assert_matches( qr/CORRECT/, $text );
    $this->assert_null( $meta->get('FORM') );
}
...
sub test_prevTopicEmptyTextSave {
    ...
    my ( $meta, $text ) =
      $this->{twiki}->{store}
      ->readTopic( undef, $this->{test_web}, 'PrevTopicEmptyTextSave' );
...
sub test_simpleFormSave {
    ...
    $this->assert( $this->{twiki}->{store}
          ->topicExists( $this->{test_web}, 'SimpleFormSave' ) );
    my ( $meta, $text ) =
      $this->{twiki}->{store}
      ->readTopic( undef, $this->{test_web}, 'SimpleFormSave' );
...
sub test_templateTopicFormSave {
    ...
    my ( $meta, $text ) =
      $this->{twiki}->{store}
      ->readTopic( undef, $this->{test_web}, 'TemplateTopicAgain' );
    $this->assert_matches( qr/Template Topic/, $text );
    $this->assert_str_equals( 'TestForm1', $meta->get('FORM')->{name} );
...
sub test_prevTopicFormSave {
    ...
    my ( $meta, $text ) =
      $this->{twiki}->{store}
      ->readTopic( undef, $this->{test_web}, 'PrevTopicFormSave' );
    $this->assert_matches( qr/Template Topic/, $text );
    $this->assert_str_equals( 'TestForm1', $meta->get('FORM')->{name} );
    $this->assert_str_equals( 'Value_1',
        $meta->get( 'FIELD', 'Select' )->{value} );
...
sub test_simpleFormSave1 {
    ...
    $this->capture( \&TWiki::UI::Save::save, $this->{twiki} );
    $this->assert( $this->{twiki}->{store}
          ->topicExists( $this->{test_web}, 'SimpleFormTopic' ) );
    my ( $meta, $text ) =
      $this->{twiki}->{store}
      ->readTopic( undef, $this->{test_web}, 'SimpleFormTopic' );
    $this->assert_str_equals( 'TestForm1', $meta->get('FORM')->{name} );
...
sub test_simpleFormSave2 {
    ...
    my $oldmeta =
      new TWiki::Meta( $this->{twiki}, $this->{test_web}, 'SimpleFormSave2' );
    my $oldtext = $testtext1;
    $this->{twiki}->{store}->extractMetaData( $oldmeta, $oldtext );
    $this->{twiki}->{store}->saveTopic( $this->{test_user_login},
        $this->{test_web}, 'SimpleFormSave2', $testform1, $oldmeta );
    my $query = new CGI(
        {
            action               => ['save'],
            text                 => [$testtext_nometa],
    ...
    $this->capture( \&TWiki::UI::Save::save, $this->{twiki} );
    $this->assert( $this->{twiki}->{store}
          ->topicExists( $this->{test_web}, 'SimpleFormSave2' ) );
    my ( $meta, $text ) =
      $this->{twiki}->{store}
      ->readTopic( undef, $this->{test_web}, 'SimpleFormSave2' );
...
sub test_simpleFormSave3 {
    ...
    my $oldmeta =
      new TWiki::Meta( $this->{twiki}, $this->{test_web}, 'SimpleFormSave3' );
    my $oldtext = $testtext1;
    $this->{twiki}->{store}->extractMetaData( $oldmeta, $oldtext );
    $this->{twiki}->{store}->saveTopic( $this->{test_user_login},
        $this->{test_web}, 'SimpleFormSave3', $testform1, $oldmeta );
    my $query = new CGI(
        {
            action               => ['save'],
            text                 => [$testtext_nometa],
    ...
    $this->capture( \&TWiki::UI::Save::save, $this->{twiki} );
    $this->assert( $this->{twiki}->{store}
          ->topicExists( $this->{test_web}, 'SimpleFormSave3' ) );
    my ( $meta, $text ) =
      $this->{twiki}->{store}
      ->readTopic( undef, $this->{test_web}, 'SimpleFormSave3' );
    $this->assert($meta);
...
sub test_templateTopicWithMeta {
    ...
    TWiki::Func::saveTopicText( $this->{test_web}, "TemplateTopic",
        $testtext1 );
    my $query = new CGI(
        {
            templatetopic => ['TemplateTopic'],
    ...
    my ( $meta, $text ) =
      $this->{twiki}->{store}
      ->readTopic( undef, $this->{test_web}, 'TemplateTopicWithMeta' );
...
sub test_merge {
    ...
    my $oldmeta =
      new TWiki::Meta( $this->{twiki}, $this->{test_web}, 'MergeSave' );
    my $oldtext = $testtext1;
    $this->{twiki}->{store}->extractMetaData( $oldmeta, $oldtext );
    $this->{twiki}->{store}->saveTopic( $this->{test_user_2_login},
        $this->{test_web}, 'MergeSave', $testform4, $oldmeta );
    my ( $meta, $text ) =
      $this->{twiki}->{store}
      ->readTopic( undef, $this->{test_web}, 'MergeSave' );
...
sub test_restoreRevision {
    ...
    my ( $meta, $text ) =
      $this->{twiki}->{store}
      ->readTopic( undef, $this->{test_web}, 'DeleteTestRestoreRevisionTopic' );
    my ( $orgDate, $orgAuth, $orgRev ) = $meta->getRevisionInfo();
    my $original = "${orgRev}_$orgDate";
...
sub test_1897 {
    ...
    $this->{twiki}->{store}->saveTopic( $this->{twiki}->{user},
        $this->{test_web}, 'MergeSave', "Smelly\ncat", $oldmeta );
    my ( $meta, $text ) =
      $this->{twiki}->{store}
      ->readTopic( undef, $this->{test_web}, 'MergeSave' );
    my ( $orgDate, $orgAuth, $orgRev ) = $meta->getRevisionInfo();
