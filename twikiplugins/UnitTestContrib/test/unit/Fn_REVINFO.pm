...
sub test_basic {
    ...
    my $ui =
      $this->{twiki}
      ->handleCommonTags( '%REVINFO%', $this->{test_web}, $this->{test_topic} );
    my $guest = TWiki::Func::getWikiName();
    unless (
        $ui =~ /^r1 - \d+ \w+ \d+ - \d+:\d+:\d+ - $this->{users_web}\.$guest$/ )
    {
        $this->assert( 0, $ui );
    }
}
...
sub test_basic2 {
    ...
    my $ui =
      $this->{twiki}
      ->handleCommonTags( '%REVINFO%', $this->{test_web}, 'GlumDrop' );
    unless ( $ui =~
...
sub test_basic3 {
    ...
    my $ui =
      $this->{twiki}
      ->handleCommonTags( '%REVINFO{topic="GlumDrop"}%', $this->{test_web},
        'GlumDrop' );
    unless ( $ui =~
/^r1 - \d+ \w+ \d+ - \d+:\d+:\d+ - $this->{users_web}\.$this->{test_user_wikiname}$/
      )
...
sub test_thisWebVars {
    ...
    my $ui =
      $this->{twiki}->handleCommonTags( '%REVINFO{topic="%BASEWEB%.GlumDrop"}%',
...
sub BROKENtest_thisTopicVars {
    ...
    my $ui = $this->{twiki}->handleCommonTags( '%REVINFO{topic="%BASETOPIC%"}%',
        $this->{test_web}, 'GlumDrop' );
    unless ( $ui =~
/^r1 - \d+ \w+ \d+ - \d+:\d+:\d+ - $this->{users_web}\.$this->{test_user_wikiname}$/
      )
...
sub BROKENtest_thisWebTopicVars {
    ...
    my $ui =
      $this->{twiki}
      ->handleCommonTags( '%REVINFO{topic="%BASEWEB%.%BASETOPIC%"}%',
        $this->{test_web}, 'GlumDrop' );
    unless ( $ui =~
/^r1 - \d+ \w+ \d+ - \d+:\d+:\d+ - $this->{users_web}\.$this->{test_user_wikiname}$/
      )
...
HERE
    ...
    my $ui =
      $this->{twiki}
      ->handleCommonTags( '%REVINFO{format="$username $wikiname"}%',
