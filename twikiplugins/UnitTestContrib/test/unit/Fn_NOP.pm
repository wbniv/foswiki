...
sub test_NOP {
    ...
    my $result =
      $this->{twiki}
      ->handleCommonTags( "%NOP%", $this->{test_web}, $this->{test_topic} );
    $this->assert_equals( '<nop>', $result );

    $result = $this->{twiki}->handleCommonTags( "%NOP{   ignore me   }%",
        $this->{test_web}, $this->{test_topic} );
    $this->assert_equals( "   ignore me   ", $result );

    $result =
      $this->{twiki}->handleCommonTags( "%NOP{%SWINE%}%", $this->{test_web},
        $this->{test_topic} );
    $this->assert_equals( "%SWINE%", $result );

    $result =
      $this->{twiki}->handleCommonTags( "%NOP{%WEB%}%", $this->{test_web},
        $this->{test_topic} );
    ...
    $result =
      $this->{twiki}->expandVariablesOnTopicCreation("%NOP{   ignore me   }%");
    $this->assert_equals( '', $result );
