...
sub test_basic {
    ...
    my $ui =
      $this->{twiki}->handleCommonTags( '%USERINFO%', $this->{test_web},
        $this->{test_topic} );
...
sub test_withWikiName {
    ...
    my $ui =
      $this->{twiki}
      ->handleCommonTags( '%USERINFO{"ScumBag"}%', $this->{test_web},
        $this->{test_topic} );
    $this->assert_str_equals(
...
sub test_withLogin {
    ...
    $TWiki::cfg{AntiSpam}{HideUserDetails} = 0;
    my $ui =
      $this->{twiki}->handleCommonTags( '%USERINFO{"scum"}%', $this->{test_web},
        $this->{test_topic} );
    $this->assert_str_equals(
        "scum, $TWiki::cfg{UsersWebName}.ScumBag, scumbag\@example.com", $ui );
}
...
'%USERINFO{"ScumBag" format="W$wikiusernameU$wikinameE$emailsG$groupsE"}%',
    ...
    $this->assert_str_equals(
"W$TWiki::cfg{UsersWebName}.ScumBagUScumBagEscumbag\@example.comGGropeGroupE",
        $ui
    );

    my $guest_ui = $this->{twiki}->handleCommonTags(
'%USERINFO{"TWikiGuest" format="W$wikiusernameU$wikinameE$emailsG$groupsE"}%',
        $this->{test_web}, $this->{test_topic}
    );
    $this->assert_str_equals(
"WTemporaryUSERINFOUsersWeb.TWikiGuestUTWikiGuestEGTWikiBaseGroup, GropeGroupE",
        $guest_ui
    );
}
