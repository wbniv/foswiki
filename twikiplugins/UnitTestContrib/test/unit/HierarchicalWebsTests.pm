...
sub set_up {
    ...
    try {
        ...
        $this->{twiki}->{store}->saveTopic(
            $this->{twiki}->{user},     $testWeb,
            $TWiki::cfg{HomeTopicName}, "SMELL"
        );
        $this->assert( $this->{twiki}->{store}
              ->topicExists( $testWeb, $TWiki::cfg{HomeTopicName} ) );

        $this->{twiki}->{store}
          ->createWeb( $this->{twiki}->{user}, $testWebSubWebPath );
        $this->assert( $this->{twiki}->{store}->webExists($testWebSubWebPath) );
        $this->{twiki}->{store}->saveTopic(
            $this->{twiki}->{user},     $testWebSubWebPath,
            $TWiki::cfg{HomeTopicName}, "SMELL"
        );
        $this->assert( $this->{twiki}->{store}
              ->topicExists( $testWebSubWebPath, $TWiki::cfg{HomeTopicName} ) );
...
sub test_createSubSubWeb {
    ...
    my $webTest = 'Item0';
    $this->{twiki}->{store}->createWeb( $user, "$testWebSubWebPath/$webTest" );
    $this->assert(
        $this->{twiki}->{store}->webExists("$testWebSubWebPath/$webTest") );
...
sub test_createSubWebTopic {
    ...
    $this->{twiki}->{store}->saveTopic( $this->{twiki}->{user},
        $testWebSubWebPath, $testTopic, "page stuff\n" );
    $this->assert(
        $this->{twiki}->{store}->topicExists( $testWebSubWebPath, $testTopic )
    );
}
...
__TOPIC__
    $this->assert(
        $this->{twiki}->{store}->topicExists( $testWebSubWebPath, $baseTopic )
    );

    # create the (included) page
    $this->{twiki}->{store}->saveTopic( $this->{twiki}->{user},
        $testWebSubWebPath, $includeTopic, $testText );
    $this->assert( $this->{twiki}->{store}
          ->topicExists( $testWebSubWebPath, $includeTopic ) );

    # verify included page's text
    {
        my ( undef, $text ) =
          $this->{twiki}->{store}
          ->readTopic( $user, $testWebSubWebPath, $includeTopic );
    ...
    # base page should evaluate (more or less) to the included page's text
    {
        my ( undef, $text ) =
          $this->{twiki}->{store}
          ->readTopic( $user, $testWebSubWebPath, $baseTopic );
        $text =
          $this->{twiki}
          ->handleCommonTags( $text, $testWebSubWebPath, $baseTopic );
        $this->assert_matches( qr/$testText\s*$/, $text );
    }
}
...
sub test_create_subweb_with_same_name_as_a_topic {
    ...
    # create the page
    $this->{twiki}->{store}->saveTopic( $this->{twiki}->{user},
        $testWebSubWebPath, $testTopic, $testText );
    $this->assert(
        $this->{twiki}->{store}->topicExists( $testWebSubWebPath, $testTopic )
    );
    ...
    my ( undef, $text ) =
      $this->{twiki}->{store}
      ->readTopic( $user, $testWebSubWebPath, $testTopic );
    $this->assert_matches( qr/$testText\s*$/, $text );

    # create the subweb with the same name as the page
    $this->{twiki}->{store}
      ->createWeb( $user, "$testWebSubWebPath/$testTopic" );
    $this->assert(
        $this->{twiki}->{store}->webExists("$testWebSubWebPath/$testTopic") );
    ...
    $this->{twiki}->{store}
      ->removeWeb( $user, "$testWebSubWebPath/$testTopic" );
    $this->assert(
        !$this->{twiki}->{store}->webExists("$testWebSubWebPath/$testTopic") );
}
...
sub test_url_parameters {
    ...
    $this->{twiki}->{store}
      ->saveTopic( $this->{twiki}->{user}, $testWeb, $testWebSubWeb, "nowt" );
...
sub test_squab_simple {
    ...
    $text =
      $this->{twiki}
      ->renderer->getRenderedVersion( $text, $testWeb, 'NonExistant' );
    $this->assert_matches(
qr!<span class="twikiNewLink">$testWeb<a.*href=".*edit$TWiki::cfg{ScriptSuffix}/$testWeb/$testWeb\?topicparent=$testWeb.NonExistant"!,
        $text
    );
}
...
sub test_squab_subweb {
    ...
    $text =
      ...
      ->renderer->getRenderedVersion( $text, $testWeb, 'NonExistant' );
    $this->assert_matches(
qr!<span class="twikiNewLink">$testWebSubWeb<a.*href=".*edit$TWiki::cfg{ScriptSuffix}/$testWeb/$testWebSubWeb\?topicparent=$testWeb.NonExistant"!,
        $text
    );
}
...
sub test_squab_subweb_full_path {
    ...
    $text =
      ...
      ->renderer->getRenderedVersion( $text, $testWeb, 'NonExistant' );
    $this->assert_matches(
qr!<span class="twikiNewLink">$testWeb.$testWebSubWeb<a.*href=".*edit$TWiki::cfg{ScriptSuffix}/$testWeb/$testWebSubWeb\?topicparent=$testWeb.NonExistant"!,
        $text
    );
}
...
sub test_squab_subweb_wih_topic {
    ...
    $this->{twiki}->{store}
      ->saveTopic( $this->{twiki}->{user}, $testWeb, $testWebSubWeb, "" );
    $this->assert(
        $this->{twiki}->{store}->topicExists( $testWeb, $testWebSubWeb ) );
    ...
    $text =
      ...
      ->renderer->getRenderedVersion( $text, $testWeb, 'NonExistant' );
    $this->assert_matches(
qr!<a href=".*view$TWiki::cfg{ScriptSuffix}/$testWeb/$testWebSubWeb" class="twikiLink">$testWebSubWeb</a>!,
        $text
    );
}
...
sub test_squab_full_path_with_topic {
    ...
    $text =
      ...
      ->renderer->getRenderedVersion( $text, $testWeb, 'NonExistant' );
    $this->assert_matches(
qr!<a href=".*view$TWiki::cfg{ScriptSuffix}/$testWeb/$testWebSubWeb" class="twikiLink">$testWeb.$testWebSubWeb</a>!,
        $text
    );
}
...
sub test_squab_path_to_topic_in_subweb {
    ...
    $text =
      ...
      ->renderer->getRenderedVersion( $text, $testWeb, 'NonExistant' );
    $this->assert_matches(
qr!<a href=".*view$TWiki::cfg{ScriptSuffix}/$testWeb/$testWebSubWeb/$TWiki::cfg{HomeTopicName}" class="twikiLink">$testWeb.$testWebSubWeb.$TWiki::cfg{HomeTopicName}</a>!,
        $text
    );
