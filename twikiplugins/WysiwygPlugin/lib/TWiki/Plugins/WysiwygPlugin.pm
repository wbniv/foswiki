...
sub postConvertURL {
        ...
        && !$parameters )
    {
        my $orig = $1;
        my ( $web, $topic ) =
          TWiki::Func::normalizeWebTopicName( $opts->{web}, $orig );
    ...
    unless ($imgMap) {
        ...
        if ($imgs) {
            while ( $imgs =~ s/src="(.*?)" alt="(.*?)"// ) {
                my ( $src, $alt ) = ( $1, $2 );
                $src =
                  TWiki::Func::expandCommonVariables( $src, $opts->{topic},
                    $opts->{web} );
                $alt .= '%' if $alt =~ /^%/;
                $imgMap->{$src} = $alt;
            }
...
sub notWysiwygEditable {
    ...
    my $calls_ok = TWiki::Func::getPreferencesValue('WYSIWYG_EDITABLE_CALLS')
      || '---';
    $calls_ok =~ s/\s//g;
...
sub RESTParameter2SiteCharSet {
    ...
    if ( $TWiki::cfg{Site}{CharSet} ) {
        $text =
          Encode::encode( $TWiki::cfg{Site}{CharSet}, $text,
            Encode::FB_PERLQQ );
    }
...
sub returnRESTResult {
    ...
    if ( $TWiki::cfg{Site}{CharSet} ) {
        $text =
          Encode::decode( $TWiki::cfg{Site}{CharSet}, $text,
            Encode::FB_PERLQQ );
    }
...
sub _restUpload {
    ...
    unless ($doPropsOnly) {
        ...
        my $maxSize = TWiki::Func::getPreferencesValue('ATTACHFILESIZELIMIT');
