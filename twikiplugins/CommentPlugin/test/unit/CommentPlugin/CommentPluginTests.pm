...
sub set_up {
    ...
    $this->{twiki}->{store}
      ->createWeb( $this->{twiki}->{user}, $this->{target_web} );
}

sub tear_down {
    my $this = shift;
    $this->{twiki}->{store}
      ->removeWeb( $this->{twiki}->{user}, $this->{target_web} );
...
sub writeTopic {
    ...
    $this->{twiki}->{store}
      ->saveTopic( $this->{twiki}->{user}, $web, $topic, $text, $meta );
}
...
sub inputTest {
    ...
    my $url =
"$TWiki::cfg{DefaultUrlHost}$TWiki::cfg{ScriptUrlPath}/save$TWiki::cfg{ScriptSuffix}/$web/$topic";
...
HERE
    ...
    my $pidx = $eidx;
    my $html =
      TWiki::Plugins::CommentPlugin::Comment::_handleInput( $sattrs,
...
# id: This gives a unique name for a COMMENT, in case you have more than one COMMENT tag in a topic (mandatory with > 1 COMMENT)
    ...
    my $pidx = 0;
    my $html = TWiki::Plugins::CommentPlugin::Comment::_handleInput(
...
HERE
    ...
    my $pidx = 0;
    my $html =
      TWiki::Plugins::CommentPlugin::Comment::_handleInput( 'nopost="on"',
        $this->{test_web}, $this->{test_topic}, \$pidx, "The Message", "",
        "bottom" );
    $this->assert_matches(
    ...
    $text =
      TWiki::Func::readTopicText( $this->{test_web}, $this->{test_topic} );
...
HERE
    ...
    my $pidx = 99;
    my $html =
      TWiki::Plugins::CommentPlugin::Comment::_handleInput( 'remove="on"',
        $this->{test_web}, $this->{test_topic}, \$pidx, "The Message", "",
        "bottom" );
    $this->assert_matches(
...
HERE
    ...
    my $pidx = 99;
    my $html =
      TWiki::Plugins::CommentPlugin::Comment::_handleInput( 'default="wibble"',
        $this->{test_web}, $this->{test_topic}, \$pidx, undef, "", "bottom" );
