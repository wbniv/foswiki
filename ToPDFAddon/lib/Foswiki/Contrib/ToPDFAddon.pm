...
package Foswiki::Contrib::ToPDFAddon;
...
sub viewPDF {
   ...
   #my $Cmd = "/usr/bin/php $pubDir/System/ToPDFAddOn/topdf.php $inputFile $outputFilename $outputDir \"$webName/$topic\" \"$userName\"";
   ...
   my $Cmd = "/usr/bin/php $pubDir/System/ToPDFAddOn/topdf.php $inputFile $outputFilename $outputDir \"$utf8webName/$utf8topic\" \"$utf8userName\" \"$headerFile\" \"$footerFile\"";
...
sub _getHeaderFile {
    ...
    my ($cfh, $path ) = tempfile('html2pdfHeaderXXXX',
                          ...
                          UNLINK => 0,
                          SUFFIX => '.html');
   my $topicAsHTML = _renderTopic($session,"System","ToPDFAddonHeader");
   print $cfh $topicAsHTML; 
   close($cfh);
   return $path;
}
...
sub _getFooterFile {
    ...
    my ($cfh, $path ) = tempfile('html2pdfFooterXXXX',
                          ...
                          UNLINK => 0,
                          SUFFIX => '.html');
   my $topicAsHTML = _renderTopic($session,"System","ToPDFAddonFooter");
   print $cfh $topicAsHTML; 
   close($cfh);
   return $path;
}
