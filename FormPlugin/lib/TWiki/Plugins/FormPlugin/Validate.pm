...
- changed the order in error checking so that blank fields get the correct suggestion for entering data. For instance an email field that is left blank should show the hint the an email address is required.

Arthur Clemens, 2007. arthur@visiblearea.com
...
use vars qw(
  ...
  %Missing	%Invalid %Blank %InvalidType @ErrorFields
);
require Exporter;
@ISA    = qw(Exporter);
@EXPORT = qw(GetFormData);
@EXPORT_OK =
  qw(%Missing %Invalid %Blank %InvalidType @ErrorFields addExtensions GetFormData CheckFormData);
%EXPORT_TAGS = (
    standard => [@EXPORT_OK],
...
my %TYPES = (
    ...
    'x' => [
        ...
        sub {
            confess
q(PANIC: Can't Happen[tm]: Sorry, but type 'x' is not supported in raw form.  See EXTENSIONS in perldoc )
              . __FILE__ . '. ';
          }
    ],
);
...
## Use this code below if the CGI object form gets changed.  Yes, we're breaking OO rules, so kill me I need the speed!
    ...
    for ( my $arg = 0 ; $arg <= $#_ ; $arg += 2 ) {
        ...
        ## Added for FormPlugin: order argument
        my ( $field, $optional, $type, $dummydelimiter, $order ) =
          ( $_[$arg] =~ /^([^:=]+)([:=]?)(\w*)([:=]?)(\w*)/ );
...
qq(Invalid arg "$_[$arg]" given to GetFormData(): No field name???);
        ...
        $TYPES{$type}
          or ( $Error = qq(Invalid type "$type" given for field "$field"),
            return );
        $type = $TYPES{$type};

        $fields{$field}{reference} = $_[ $arg + 1 ]
          or ( $Error = qq(No place given to stick the value for "$field"),
            return );
        ## Check for correct reference type
        ( ref $fields{$field}{reference} eq 'SCALAR' )
          || ( ref $fields{$field}{reference} eq 'ARRAY' )
          or ( $Error =
qq(Invalid reference type "@{[ ref ($fields{$field}{reference}) ]}" given for "$field".  Must be SCALAR or ARRAY),
            return );
    ...
    foreach my $field ( $query->param ) {
        ...
        unless ( exists $fields{$field} ) {
            ...
            unless ($IgnoreNonMatchingFields) {
                ...
                _addErrorField( $field, 'invalid', $fields{$field}{order} );
            }
            next;
        }
        ...
        foreach my $arg ( @{ $form{$field} } ) {
            ...
            # right type?
            if ( !$fields{$field}{type}[1]->($arg) ) {

                # not the right type
                # do we expect a type?
                my $expected = $fields{$field}{type}[0];
                if ( length $arg == 0 && $expected eq 'string' ) {
...
qq(Required field "$field" contains no data in $argNum segment);
                            ...
                            $Blank{$field} =
                              qq(Required field "$field" contains no data);
                            _addErrorField( $field, 'blank',
                                $fields{$field}{order} );
                        }
                    }
...
qq(Invalid data type found for array field $field, indices $argNum ($fields{$field}{type}[0] expected, found "$arg"));
                    ...
                    $InvalidType{$field} =
qq(Invalid data type found for field $field ($fields{$field}{type}[0] expected, found "$arg"));
                    _addErrorField(
                        $field, 'invalidtype',
                        $fields{$field}{order},
                        $fields{$field}{type}[0]
    ...
    if ( %Missing or %Invalid or %Blank or %InvalidType ) {
        $Error = join ",\n", values %Missing, values %Invalid, values %Blank,
          values %InvalidType;
        return;
    }
    else {
...
sub _addErrorField {
    my ( $field, $errorType, $order, $expected ) = @_;
    push @ErrorFields,
      {
        field    => $field,
        type     => $errorType,
