...
sub _acceptA {
    ...
    my $merged =
      $session->{plugins}->mergeHandler( ' ', $ai->[$a], undef, $info );
    if ( defined $merged ) {
        push( @$out, $merged );
    }
    else {
...
sub _acceptB {
    ...
    my $merged =
      $session->{plugins}->mergeHandler( ' ', $bi->[$b], undef, $info );
    if ( defined $merged ) {
        push( @$out, $merged );
    }
    else {
...
sub _change {
    ...
    if ( $ai->[$a] =~ /\S/ ) {
        ...
        # inserting new
        $merged =
          $session->{plugins}->mergeHandler( '+', $ai->[$a], $bi->[$b], $info );
...
sub _handleConflict {
    ...
    my $merged =
      $session->{plugins}
      ->mergeHandler( 'c', join( '', @b ), join( '', @c ), $info );
    if ( defined $merged ) {
        push( @$out, $merged );
    }
    else {
