...
BEGIN {
    ...
    $VERSION = '$Date: 2008-10-13 22:21:28 -0700 (Mon, 13 Oct 2008) $ $Rev: 17624 $ ';
...
sub redirect {
    ...
    return
      if ( $this->{users}->{loginManager}->redirectCgiQuery( $query, $url ) );
...
sub new {
    ...
    if ($topic) {
            ...
            && $this->{cgiQuery} )
        {
            ...
            exit
              ; #we seriously don't want to go through normal TWiki operations if we're redirecting..
        }
        elsif ( $topic =~ /((?:.*[\.\/])+)(.*)/ ) {
