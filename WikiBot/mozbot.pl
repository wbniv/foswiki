...
sub targetted {
    ...
    if ( exists $commandChar{$chan} ) { $cmdChar = $commandChar{$chan}; }
    
    if ($data =~ /^(\s*$nick(?:[\s,:;.!?]+|\s*:-\s*|\s*--+\s*|\s*-+>?\s+))(.+)$/is) {
      $ret = (defined $2 ? $2 : '');
    } else {
      if ($data =~ /^(\s*$cmdChar)(.+)$/is) {
        $ret = (defined $2 ? $2 : '');
      } else {
        $ret = undef;
      }
...
sub on_public {
    ...
    if (defined($_ = targetted($data, quotemeta($nicks[$nick]), $channel))) {
        if ($_ ne '') {
        ...
        foreach my $nick (@ignoredTargets) {
            if (defined targetted($data, $nick, $channel)) {
