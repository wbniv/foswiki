...
{
	my( $self, $text ) = @_;
	if ($self->{output_module}->{html}) { $text =~ s/&/&amp;/g; }
	$self->{context} = new Beautifier::Context();
	...
	my $out = "";
	if (defined($self->{context}->{code_parts}[0])) { $out .= $self->{context}->{code_parts}[0]; }
	my $lineout = "";
...
sub starts_with
{
	...
	{
		$l = length($_);
		if (((!$self->{highlightfile}->{nocase} && substr($text, 0, $l) eq $_) || ($self->{highlightfile}->{nocase} && lc(substr($text, 0, $l)) eq lc($_))) && (substr($text, $l, 1) eq " " || $l == 1 || @{$text}[$l] eq "\n" || @{$text}[$l] == "\t" || @{$text}[$l] eq "." || @{$text}[$l] eq ";" || $l == length($text)))
		{
			if ($l > $ml)
			{
			...
			if ($delim || $i==$lngth)
			{
				if ($inword)
				{
					...
					if (defined($self->{context}->{validkeys}{$checkword}))
					{
						...
						# [LEAF] made changes to get linkscripts working.
