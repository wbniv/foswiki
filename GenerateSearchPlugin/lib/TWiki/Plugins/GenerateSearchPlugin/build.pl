...
{ package GenerateSearchPluginBuild;

  @GenerateSearchPluginBuild::ISA = ( "TWiki::Contrib::Build" );

  sub new {
    my $class = shift;
    return bless( $class->SUPER::new( "GenerateSearchPlugin", "Build" ), $class );
  }
...
$build = new GenerateSearchPluginBuild();
