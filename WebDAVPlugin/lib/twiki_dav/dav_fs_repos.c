...
static dav_error * dav_fs_copymove_file(
    ...
    const char *src,
    const char *dst,
    const struct stat *src_finfo,
    const struct stat *dst_finfo,
    dav_buffer *pbuf)
{
    ...
    const char *workspace)
{
    ...
    */
    ...
    return dav_fs_get_twiki_info(r, resource);
}
					  ...
					  dav_text_header *phdr)
{
	...
	*/
	...
	(void) sprintf(buf, "%ld",
                       (unsigned long) resource->info->finfo.st_size);
	value = buf;
	break;
