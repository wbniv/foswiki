...
use APR::Table;
use ModPerl::Util;
use Apache2::Const qw(DECLINED HTTP_NO_CONTENT
                      HTTP_METHOD_NOT_ALLOWED NOT_FOUND OK FORBIDDEN);
use Data::Dumper;
use Encode;
use File::Spec;
...
my %implemented = (
    ...
    #post     => 1,
    #trace    => 1,
    #lock     => 1, # implement as topic lock?
    #unlock   => 1,
);
...
sub process {
    ...
    if ($implemented{$method}) {
        ...
        print STDERR "$method DECLINED on $uri\n";
        return DECLINED;
    }
}
...
# 
#     $r->status(200);
#     $r->headers_out->set("Allow" =>
#                    "OPTIONS, HEAD, GET, PUT, " .
#                    "DELETE, MKCOL, PROPPATCH, PROPFIND, COPY, MOVE");
#     $r->headers_out->set("DAV" => "1,<http://apache.org/dav/propset/fs/1>");
#     $r->send_http_header();
# 
#     return OK;
# }
...
sub copy {
    ...
    my $depth       = $r->headers_in->get('Depth');
    my $overwrite   = $r->headers_in->get('Overwrite');
    ...
    my @files = map { s|/+|/|g; $_ }
        File::Find::Rule::Filesys::Virtual->virtual($handler)
...
sub get {
    ...
    if ($handler->test('f', $path) && $handler->test('r', $path)) {
        $r->headers_out->set('Last-Modified' => ''.$handler->modtime($path));
        ...
        $r->status(200);
        $r->headers_out->set('Content-Length' => length($file));

        #MP1 $r->send_http_header();
        $r->print($file);
    ...
    # structure.
    #
    # This happens if you connect with a regular browser, or if you
    # connect using IE but don't check the Web Folder box.  So print a
    # warning.
    #
    # Generate a simple dir listing with links.
    if ($handler->test('d', $path) && $handler->test('r', $path)) {
        $r->content_type('text/html; charset="utf-8"');
        $r->print('<html><body><code>');
        #MP1 $r->send_http_header();
        my @contents = $handler->list($path);
        foreach my $file (@contents) {
            my $url = $path.'/'.$file;
            $r->print("<a href='$url'>$file</a><br />");
        }
        $r->print("</code><p /><small>Note: If you are using IE, please use
...
sub options {
    ...
    $r->headers_out->set('Allow'         => join(',', map { uc } keys %implemented));
    $r->headers_out->set('DAV'           => '1,2,<http://apache.org/dav/propset/fs/1>');
    $r->headers_out->set('MS-Author-Via' => 'DAV');
    $r->headers_out->set('Keep-Alive'    => 'timeout=15, max=96');
...
sub propfind {
    ...
    if ($depth == 0) {
        ...
        # response.
        if ($r->headers_in->get('User-Agent') !~ /Goliath/) {
            push @files, $uri;
        }
    }
...
sub delete_response {
    ...
    #
    if ($r->headers_in->get('User-Agent') =~ /WebDrive/) {
        $r->status(FORBIDDEN);
        #MP1 $r->send_http_header();
        return OK;
    }
...
Copyright (C) 2008 C-Dot Consultants L<http://c-dot.co.uk>
